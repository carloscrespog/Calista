 ...
 */
public class ScriptletProcessor_V2 extends AbstractVirtualSensor {

    private static final transient Logger logger = Logger.getLogger(ScriptletProcessor_V2.class);
     ...
     * the {@link gsi.calista.processor.ScriptletProcessor_V2#scriptlet}
     * and {@link gsi.calista.processor.ScriptletProcessor_V2#scriptletPeriodic} fields.
     */
    ...
    public void dataAvailable(String inputStreamName, StreamElement se) {
        evaluate(scriptlet,inputStreamName, se, persistant);
    }
    ...
    protected boolean initialize(DataField[] outputStructure, TreeMap<String, String> parameters) {
        ...
        if (scriptletPeriodic != null && period >= 0) {
            periodicalTask = new TimerTask() {
                public void run() {
                    evaluate(scriptletPeriodic, null,null, false);
                }
            };
    ...
    protected Script initScriptlet(String ps) {
        ...
        scriptlet.append("import gsn.beans.*;\n");
        scriptlet.append("import gsn.Mappings;\n");
    ...
    protected Binding updateContext(String inputStreamName, StreamElement se) {
        if (se != null) {
            for (String fieldName : se.getFieldNames()) {

            	context.setVariable("STREAM", inputStreamName);
    ...
    protected void evaluate(Script script, String inputStreamName, StreamElement se, boolean persist) {
        StreamElement seo = null;
        synchronized (context) {
            updateContext(inputStreamName,se);
